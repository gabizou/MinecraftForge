...
import net.minecraftforge.event.old.terraingen.DecorateBiomeEvent.Decorate;
import net.minecraftforge.event.world.gen.GenerateOreEvent.GenerateMinable;
import net.minecraftforge.event.old.terraingen.PopulateChunkEvent.Populate;
import net.minecraftforge.event.old.terraingen.SaplingGrowTreeEvent;
...
{
    public static <T extends InitializeNoiseGeneratorsEvent.Context> T getModdedNoiseGenerators(World world, Random rand, T original)
    {
        InitializeNoiseGeneratorsEvent<T> event = new InitializeNoiseGeneratorsEvent<>(world, rand, original);
        MinecraftForge.TERRAIN_GEN_BUS.post(event);
        return event.getNewValues();
    }

    public static MapGenBase getModdedMapGen(IChunkGenerator chunkGen, MapGenBase original, InitializeMapGeneratorFeaturesEvent.EventType type)
    {
        InitializeMapGeneratorFeaturesEvent event = new InitializeMapGeneratorFeaturesEvent(Cause.of(chunkGen), type, original);
        MinecraftForge.TERRAIN_GEN_BUS.post(event);
    ...
    {
        GenerateMinable event = new GenerateMinable(Cause.of(generator, world), world, rand, generator, pos, type);
